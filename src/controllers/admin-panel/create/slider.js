...
const createSlider = async (req, res) => {
    ...
    if (!files) {
        ...
        if (path.extname(files.file.name) === '.jpg' || path.extname(files.file.name) === '.jpeg' || path.extname(files.file.name) === '.png') {
            ...
            const slider = await Slider.create({ picture: fileName + extension, createdBy })
            Log.create({ logTypeId: tokenData.logTypeId, description: `Slider Successfully Added By ${tokenData.name}`, createdBy: tokenData.id })
